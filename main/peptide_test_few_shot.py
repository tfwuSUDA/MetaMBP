...
class SelfAttention(nn.Module):
    def __init__(self, num_attention_heads, input_size, hidden_size, hidden_dropout_prob, max_position_embeddings=512):
        ...
        self.position_embeddings = nn.Parameter(self._get_sinusoidal_position_encoding(max_position_embeddings, input_size), requires_grad=False)

    def _get_sinusoidal_position_encoding(self, num_positions, embedding_dim):
        """Generate sinusoidal position encoding for input embeddings."""
        position = torch.arange(0, num_positions, dtype=torch.float).unsqueeze(1)
        div_term = torch.exp(torch.arange(0, embedding_dim, 2).float() * (-math.log(10000.0) / embedding_dim))
        pos_enc = torch.zeros(num_positions, embedding_dim)
    ...
    def forward(self, input_tensor, mask=None):
        ...
        position_encodings = self.position_embeddings[:, :seq_length, :].to(input_tensor.device)
        input_tensor = input_tensor + position_encodings

        # Standard self-attention calculations
        ...
        # Output hidden states with residual connection and LayerNorm
...
def main(args):
    ...
    for i in range(TEST_EPISODE):
        ...
        with torch.no_grad():
            ...
            query_features = feature_encoder(Variable(query_inputs).cuda(GPU)) # 20x64
