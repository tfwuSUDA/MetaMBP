...
class SelfAttention(nn.Module):
    ...
    def forward(self, input_tensor, mask= None):
        ...
        return attention_probs, hidden_states
...
class TextCNNEncoder(nn.Module):
    ...
    def forward(self,x):
        ...
        x = self.embedding(x)
        # attn
        attns, x = self.attn(x, mask.unsqueeze(1).unsqueeze(2))
        ...
        x = torch.index_select(x, 0, un_idx.to(device))
        ...
        x = torch.cat(x, 1).squeeze(-1)  # [256, 256]
...
def genIdxforMultiLabel(file, max_len):
    ...
    with open(file, 'r') as inf:
        ...
        labels = []
        pep_seq = []
        max_seq_len = max_len  # 设置你希望的最大序列长度
        for pep in lines:
            _, label, pep = pep.split("\t")
            if pep=="sequence":
                continue
            labels.append(list(map(int,label.split(";"))))
            input_seq = ''.join(pep)
            # 填充或截断蛋白质序列到指定的 max_seq_len
            if len(pep) > max_seq_len:
                pep = pep[:max_seq_len]
                long_pep_counter += 1
            else:
                pep = pep + '0' * (max_seq_len - len(pep))
            current_pep = []
            for aa in pep:
                current_pep.append(aa_dict[aa])
            pep_codes.append(torch.tensor(current_pep))
        print("length > {0}:".format(max_seq_len), long_pep_counter)
        idx = rnn_utils.pad_sequence(pep_codes, batch_first=True)
        return idx, torch.tensor(labels, dtype=torch.float)
...
def train(args):
    ...
    model                    = TextCNNEncoder(32, 128).to(device)
    if args.use_meta:
        model                = load_params(model, meta_model_path, device=device)
        print('using meta')
    else:
        print('no meta')
    # model                = torch.load(meta_model_path).to(device)
    ...
    for epoch in progress_bar:
        ...
        for idxes, labels in trainloader:
            optimizer.zero_grad()
            _, output       = model(idxes.to(device))
        ...
        with torch.no_grad():
            ...
            for idxes, labels in testloader:
                attn_scores, output          = model(idxes.to(device))
...
if __name__ == '__main__':
    ...
    parser.add_argument('-meta_model', type=str, default="/home/dell/disks/zeno/FewShotLearning/LearningToCompare_FSL-master/peptides/models/old/peptide_feature_encoder_32filters_5way_5shot.pkl",
